...
// offsets into 512-byte identification space
...
struct ideDevice
{
    UInt8 reserved;     // 0 = nope, 1 = device exists
    UInt8 channel;      // 0 = primary, 1 = secondary
    UInt8 drive;        // 0 = master, 1 = slave
    UInt16 type;        // 0 = ATA, 1 = ATAPI (WHY is this 16 bits??!)
    UInt16 signature;   // drive signature
    UInt16 caps;        // drive capabilities (features)
    UInt32 cmdSets;     // command sets supported by drive
    UInt32 size;        // disk size in sectors
    UInt8 model[41];    // device model string
};
